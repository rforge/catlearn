...
colnames(aggdata) <- c('stim', 'partic', 'resp')

# Pull out data from first participant:
obs1 <- aggdata[aggdata[ ,'partic'] == 1, 'resp']
...
# Define number of stimuli and participants:
nstim <- 15
nppts <- 40

# Create vector of participant numbers:
ppts <- rep(1:nppts, each = nstim)
...
rm(nstim, ppts, testdata, aggdata, tpred, nppts, nstim)

# Calculate obs1-exp1 SSE:
ssecl(obs1,exp1)
...
sse <- NULL
for (i in 1:ppts){
  obs <- aggdata[aggdata[ ,'partic'] == i, 'resp']
  exp <- tpred[tpred[ ,"ppts"] == i, 11]
  sse.i <- ssecl(obs,exp)
  sse <- rbind(sse, sse.i)
}
