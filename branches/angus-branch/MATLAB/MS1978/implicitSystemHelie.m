function [OutputImplicit,strengthWeights]=implicitSystemHelie(Parameters,CategoryStructure,NumberTimesAround)
%% Defaults
  if nargin <4
...
%% Trials
    for trial=1:size(randomisedTrials,1)
        stimulus=stimuli(:,randomisedTrials(trial,1)); % Column 8 long
...
%% Calculating the reward prediction error on this trial
        % Actual reward
        if OutputImplicit(trial,6)==OutputImplicit(trial,2)
