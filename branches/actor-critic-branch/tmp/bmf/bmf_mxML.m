function BMF = bmf_mxML(data, hfunc, PSpec, descrip, numinit_g)
...
[nprm, pfree, pconst, nsubjs, rng, pmx_i,...
    pfree_i, pfree_g, rng_g, pA_g, pb_g, rng_i, pA_i, pb_i] = ...
...
hmedfunc    = @(x)medfunc(x,pconst,pfree,pfree_g,pfree_i,hfunc,data,hoptimrep_i);
...
% sub functions
%--------------------------------------------------------------------------
function nll = medfunc(prms_g,pconst,pfree,pfree_g,pfree_i,hfunc,data,hoptimrep_i)
    global P_g P_i NLL_g;
    ...
    nll_i  = nan(1,ns);
    p_i = nan(sum(pfree_i),ns);
%     p_g = nan(sum(pfree_g),1);
    for s=1:ns
        hmedfunc_i = @(x)medfunc_subj(x,pconst_i,pfree_i,hfunc,data{s});
...
function nll = medfunc_subj(prms,pconst,pfree,hfunc,data)
