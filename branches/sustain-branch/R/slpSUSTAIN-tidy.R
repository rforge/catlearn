...
.cluster.activation <- function(lambda, r, beta, mu) {
    mu.neg <- sweep(mu, MARGIN = 2, -lambda, `*`)
    nom <- sweep(exp(mu.neg), MARGIN = 2, lambda ^ r, `*`)  
...
slpSUSTAIN <- function(st, tr, xtdo = FALSE) {
    ...
    for (i in 1:nrow(tr)) {
        ...
        ## Reset network is requested to do so.
        if (trial['ctrl'] == 1) {
            cluster <- matrix(as.vector(trial[st$colskip:(length(trial)-1)]),
                              nrow = 1)
            ## AW: Note, the above line of code will need to change once
            ## 'cluster' is part of st.
            w <- st$w
            lambda <- st$lambda
        }
