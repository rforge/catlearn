...
.output.store <- function(prob.o, que.pattern) {
  inp <- list(que.pattern, prob.o)
  ord <- sort(rep(seq(ncol(prob.o)), length(inp)))
  n <- max(unlist(lapply(inp, length)))
  prob.xout <- sapply(inp, `[`, 1:n)
  browser()  
  return(prob.xout)
}
...
slpSUSTAIN <- function(st, tr, xtdo = FALSE) {
 ...
 # Equation 13
   lambda <- lambda + (st$eta * e ^ (mu.product.neg[win, ]) * 
 ...
 # Equation 14 - one-layer delta learning rule (Widrow & Hoff, 1960)
  ...
  # browser()
  prob.xout <- .output.store(prob.o, tr[, st$colskip - 1 + fac.queried])
